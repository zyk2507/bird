  ...
  *tx->buf.pos++ = f->code;
  ...
  for (uint i = 0; i < f->optcnt; i++)
  {
	...
	*tx->buf.pos++ = 0xff;
	...
	uint cp = MIN(tx->buf.end - tx->buf.pos, opt->len - d);
    ...
    if (tx->buf.pos + rlen > tx->buf.end)
      bug("This CoAP option (size %u) is too large for one TX block", rlen);
...
coap_bad_request(struct coap_session *s, const char *fmt, ...)
{
  ...
  coap_tx_send(s, COAP_TX_FRAME(COAP_SCO_ABORT, NULL));
...
static bool
coap_process_sco_ping(struct coap_session *s UNUSED)
{ return false; }
...
static bool
coap_process_sco_abort(struct coap_session *s UNUSED)
{ return false; }
