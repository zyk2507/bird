...
#define COAP_TX_FRAME(_code, ...) ({ \
    struct coap_tx_option *_opts[] = { __VA_ARGS__ }; \
    uint _optcnt = ARRAY_SIZE(_opts); \
    struct coap_tx_frame *_f = alloca(sizeof(struct coap_tx_frame) + _optcnt * sizeof(struct coap_tx_option *)); \
    bzero(_f, sizeof(struct coap_tx_frame) + _optcnt * sizeof(struct coap_tx_option *)); \
    _f->code = _code; \
    _f->optcnt = _optcnt; \
    for (uint _i = 0; _i < _optcnt; _i++) \
      _f->opt[_i] = _opts[_i]; \
    _f; })
