...
static void
bgp_damp_clear_ea(ea_list **slot)
{
  if (!*slot)
    return;
...
static void
bgp_damp_store_ea(ea_list **slot, ea_list *attrs)
{
  ASSERT(attrs->stored);
...
bgp_damp_release_route(struct bgp_damp_info *bdi)
{
  ...
  if (!bdi->stored_ea || (c->c.channel_state != CS_UP))
    return;

  rte e = {
    .attrs = ea_ref_tmp(bdi->stored_ea),
    .src = bdi->src,
  };

  rte_update(&c->c, bdi->net, &e, bdi->src);
}
...
bgp_damp_info_free(struct bgp_damp_info *bdi, int release_route)
{
  ...
  bgp_damp_clear_ea(&bdi->last_ea);
  bgp_damp_clear_ea(&bdi->stored_ea);
...
bgp_damp_has_visible_route(struct bgp_channel *c, const net_addr *n,
			   struct rte_src *src)
{
  struct rt_export_feed *feed = rt_net_feed(c->c.table, n, NULL);
  ...
  {
    rte *old = &feed->block[i];

    if (old->flags & REF_OBSOLETE)
      return 0;

    if (rte_is_valid(old) && (old->src == src))
      return 1;
  }
		...
		struct rte_src *src, ea_list *attrs)
{
  ...
  if (bdi && bdi->last_ea && (bdi->last_ea != attrs))
  ...
  {
    bgp_damp_store_ea(&bdi->last_ea, attrs);
    bgp_damp_store_ea(&bdi->stored_ea, attrs);
