...
{
  struct cbor_writer *writer = lp_alloc(lp, sizeof(struct cbor_writer));
  writer->cbor = buff;
...
void cbor_add_ipv4_prefix(struct cbor_writer *writer, const net_addr_ip4 *n)
{
  write_item(writer, 6, 52); // 6 is TAG, 52 is tag number for ipv4
...
void cbor_add_ipv6_prefix(struct cbor_writer *writer, const net_addr_ip6 *n)
{
  write_item(writer, 6, 54); // 6 is TAG, 54 is tag number for ipv6
  cbor_open_block_with_length(writer, 2);
  cbor_add_int(writer, n->pxlen);
...
void check_memory(struct cbor_writer *writer, int add_size)
{
  if ((add_size < 0) || (writer->pt + (uint) add_size > writer->capacity))
  {
...
uint compare_buff_str(struct buff_reader *buf_read, uint length, const char *string) {
  uint string_len = strlen(string);
  if ((length != string_len) || (buf_read->pt + length > buf_read->size)) {
    return 0;
  }
...
{
  struct value val = { .major = CBOR_FLOAT, .val = -1 };
  if (reader->pt >= reader->size)
    return val;
  ...
  {
    if (reader->pt + 1 >= reader->size)
    {
      reader->pt = reader->size;
      return val;
    }
